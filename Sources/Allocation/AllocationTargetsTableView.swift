...
public struct AllocationTargetsTableView: View {
    ...
    @State private var currentFindings: [ValidationFinding] = []
    ...
    public var body: some View {
                        ...
                        HStack {
                            ...
                            if sub.validationStatus != "compliant" {
                                Button("Why?") {
                                    ...
                                    let findings = viewModel.findings(for: scope, db: db)
                                    guard !findings.isEmpty else {
                                        viewModel.load(using: db)
                                        return
                                    }
        ...
        .sheet(item: $presentedScope) { scope in
            ValidationDetailsView(scope: scope, findings: currentFindings)
        }
    }
