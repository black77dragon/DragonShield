        ...
        if sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
            ...
            while sqlite3_step(stmt) == SQLITE_ROW {
                ...
                if nativeValue == 0 {
                    status = "No position"
                    noPos += 1
                } else if let conv = dbManager.convertToBase(amount: nativeValue, currencyCode: currency, asOf: positionsAsOf) {
                    valueBase = conv.value
                    if conv.rateDate > (fxAsOf ?? .distantPast) { fxAsOf = conv.rateDate }
                    included += 1
                } else {
