...
final class PortfolioValuationService {
    ...
    private let fxConverter: FXConverter
    private static let dateFormatter = ISO8601DateFormatter()

    init(dbManager: DatabaseManager) {
        self.dbManager = dbManager
        self.fxConverter = FXConverter(dbManager: dbManager)
    }
        ...
        if sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
            ...
            while sqlite3_step(stmt) == SQLITE_ROW {
                ...
                if nativeValue == 0 {
                    status = "No position"
                    noPos += 1
                } else if let converted = fxConverter.convert(value: nativeValue, from: currency, to: dbManager.baseCurrency, asOf: positionsAsOf, fxAsOf: &fxAsOf) {
                    valueBase = converted
                    included += 1
                } else {
