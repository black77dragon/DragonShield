...
struct AllocationTreeCard: View {
    ...
    var body: some View {
        Card {
            GeometryReader { geo in
                ...
                let devW = geo.size.width * 0.30

                VStack(spacing: 0) {
                    HeaderBar()
                    CaptionRow(nameWidth: nameW - 16,
                               targetWidth: targetW,
                               actualWidth: actualW,
                               deviationWidth: devW,
                               gap: gap)
                    Divider()
                    ScrollView { VStack(spacing: 0) { rows(nameW - 16, targetW, actualW, devW) } }
                }
            }
        }
    ...
    @ViewBuilder
    private func rows(_ nameW: CGFloat, _ targetW: CGFloat, _ actualW: CGFloat, _ devW: CGFloat) -> some View {
...
struct AssetRow: View {
    ...
    private var trackWidth: CGFloat { max(deviationWidth - labelWidth, 0) }
    ...
    var body: some View {
        ...
        HStack(spacing: gap) {
            ...
            Text(formatValue(target, width: targetWidth))
                .frame(width: targetWidth, alignment: .trailing)
                .font(node.children != nil ? .body.bold() : .subheadline)
                .lineLimit(1)
            Text(formatValue(actual, width: actualWidth))
                .frame(width: actualWidth, alignment: .trailing)
                .font(node.children != nil ? .body.bold() : .subheadline)
                .lineLimit(1)
            ...
            HStack(spacing: labelInside ? 0 : 4) {
                ZStack(alignment: diffPct >= 0 ? .trailing : .leading) {
                    ...
                    if labelInside {
                        Text(formatDeviation(deviation, width: deviationWidth))
                            .font(.caption2)
    ...
    private func shortChf(_ value: Double) -> String {
        let absV = abs(value)
        if absV >= 1_000_000 {
    ...
    private func formatSignedShortChf(_ value: Double) -> String {
        let sign = value >= 0 ? "+" : "-"
        return sign + shortChf(abs(value))
    }

    private func formatValue(_ value: Double, width: CGFloat) -> String {
        if mode == .percent { return formatPercent(value) }
        return width < 80 ? shortChf(value) : formatChf(value)
    }

    private func formatDeviation(_ value: Double, width: CGFloat) -> String {
        if mode == .percent { return formatSignedPercent(value) }
        return width < 80 ? formatSignedShortChf(value) : formatSignedChf(value)
    }
