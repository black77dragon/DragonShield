    ...
    var body: some View {
        Card {
            GeometryReader { geo in
                ...
                var nameCol = remaining - targetCol - actualCol

                if nameCol < 16 {
                    let deficit = 16 - nameCol
                    let reduceTarget = min(deficit / 2, targetCol - minValue)
                    targetCol -= reduceTarget
                    let reduceActual = min(deficit - reduceTarget, actualCol - minValue)
                    actualCol -= reduceActual
                    nameCol = 16
                }

                nameCol = max(16, nameCol)
