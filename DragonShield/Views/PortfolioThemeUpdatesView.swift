...
struct PortfolioThemeUpdatesView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            List(selection: $selectedId) {
                ForEach(updates) { update in
                    ...
                    .contextMenu {
                        ...
                        if update.pinned {
                            Button("Unpin") { togglePin(for: update, to: false, source: nil) }
                        } else {
                            Button("Pin") { togglePin(for: update, to: true, source: nil) }
                        }
                        Button("Delete", role: .destructive) { deleteUpdate(update, source: nil) }
                    }
                }
            }
            Divider()
            HStack {
                ...
                Button(selectedUpdate?.pinned == true ? "Unpin" : "Pin") {
                    if let u = selectedUpdate { togglePin(for: u, to: !u.pinned, source: "footer") }
                }
                    .disabled(selectedUpdate == nil)
            }
            .padding(8)
        ...
        .alert(errorMessage, isPresented: $showError) { Button("OK", role: .cancel) { } }
    }
    ...
    private func deleteSelected() {
        if let u = selectedUpdate { deleteUpdate(u, source: "footer") }
    }

    private func togglePin(for update: PortfolioThemeUpdate, to pinned: Bool, source: String?) {
        DispatchQueue.global(qos: .userInitiated).async {
            if let result = dbManager.updateThemeUpdate(id: update.id, title: nil, bodyMarkdown: nil, type: nil, pinned: pinned, actor: NSFullUserName(), expectedUpdatedAt: update.updatedAt, source: source) {
                DispatchQueue.main.async {
                    load()
                    if source == "footer" { selectedId = result.id }
                }
            } else {
                DispatchQueue.main.async {
                    errorMessage = "Failed to \(pinned ? "pin" : "unpin") update"
                    showError = true
                }
            }
        }
    ...
    private func deleteUpdate(_ update: PortfolioThemeUpdate, source: String?) {
        DispatchQueue.global(qos: .userInitiated).async {
            if dbManager.deleteThemeUpdate(id: update.id, themeId: themeId, actor: NSFullUserName(), source: source) {
                DispatchQueue.main.async {
                    load()
                    if source == "footer" { selectedId = nil }
                }
            } else {
                DispatchQueue.main.async {
