...
// (removed old cards layout)
    ...
    private extension AlertsTimelineView {
        func buildItems() -> [TimelineItem] {
            ...
            for (aid, name, sev, d) in eventsRaw {
                items.append(TimelineItem(id: items.count + 1, refIndex: 0, when: d, series: "Events", alertId: aid, alertName: name, severity: sev))
            }
            let eventIds = Set(eventsRaw.map { $0.0 })
            for (aid, name, sev, d) in upcomingRaw where !eventIds.contains(aid) {
                items.append(TimelineItem(id: items.count + 1, refIndex: 0, when: d, series: "Upcoming", alertId: aid, alertName: name, severity: sev))
            }
            items.sort { a, b in
                if a.when == b.when { return a.series < b.series } else { return a.when < b.when }
            }
            for i in 0 ..< items.count {
                items[i] = TimelineItem(id: items[i].id, refIndex: i + 1, when: items[i].when, series: items[i].series, alertId: items[i].alertId, alertName: items[i].alertName, severity: items[i].severity)
            }
            return items
        }
    ...
    private struct TimelineDetailsTable: View {
        ...
        var onOpen: ((Int) -> Void)? = nil
        #if os(macOS)
            @AppStorage("timelineTableColumnWidths") private var storedTimelineTableColumnWidths: Data = .init()
