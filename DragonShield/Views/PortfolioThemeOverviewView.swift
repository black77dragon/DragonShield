...
struct PortfolioThemeOverviewView: View {
    ...
    private func indicatorRow(_ update: PortfolioThemeUpdate) -> some View {
        ...
        var items: [Indicator] = []
        if FeatureFlags.portfolioAttachmentsEnabled(), let atts = extra?.attachments {
            items += atts.map { .file($0) }
        }
        if FeatureFlags.portfolioLinksEnabled(), let links = extra?.links {
    ...
    private func expandedDetails(_ update: PortfolioThemeUpdate) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            if FeatureFlags.portfolioLinksEnabled(), let links = extra?.links, !links.isEmpty {
            ...
            if FeatureFlags.portfolioAttachmentsEnabled(), let atts = extra?.attachments, !atts.isEmpty {
    ...
    private func load() {
        ...
        for upd in list {
            let links = FeatureFlags.portfolioLinksEnabled() ? linkRepo.listLinks(updateId: upd.id) : []
