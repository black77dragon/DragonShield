...
final class AttachmentService {
    ...
    func quickLook(attachmentId: Int) {
        ...
        var previewURL = base
        if let e = ext, !e.isEmpty {
            let tmp = fm.temporaryDirectory.appendingPathComponent(UUID().uuidString).appendingPathExtension(e)
            try? fm.linkItem(at: base, to: tmp)
            previewURL = tmp
        }
        let quickLookApp = URL(fileURLWithPath: "/System/Library/CoreServices/Quick Look.app")
        let config = NSWorkspace.OpenConfiguration()
        NSWorkspace.shared.open([previewURL], withApplicationAt: quickLookApp, configuration: config) { _, _ in
            if previewURL != base {
                try? self.fm.removeItem(at: previewURL)
            }
        }
    }
