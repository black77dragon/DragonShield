...
final class AttachmentService {
    ...
    func quickLook(attachmentId: Int) {
        ...
        let sql = "SELECT sha256, original_filename FROM Attachment WHERE id = ?"
        var stmt: OpaquePointer?
        ...
        sqlite3_finalize(stmt)
        guard let hash = sha, let filename = name else { return }
        let source = attachmentsDir.appendingPathComponent(String(hash.prefix(2))).appendingPathComponent(hash)
        let temp = FileManager.default.temporaryDirectory.appendingPathComponent(filename)
        try? FileManager.default.removeItem(at: temp)
        try? FileManager.default.createSymbolicLink(at: temp, withDestinationURL: source)
        AttachmentService.previewCoordinator.previewURL = temp as NSURL
        if let panel = QLPreviewPanel.shared() {
            panel.dataSource = AttachmentService.previewCoordinator
    ...
    private static let previewCoordinator = PreviewCoordinator()
    private class PreviewCoordinator: NSObject, QLPreviewPanelDataSource {
        ...
        func previewPanel(_ panel: QLPreviewPanel!, previewItemAt index: Int) -> QLPreviewItem! { previewURL }
    }
}
