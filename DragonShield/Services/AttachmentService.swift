...
final class AttachmentService {
    ...
    func ingest(fileURL: URL, actor: String) -> Attachment? {
        ...
        case .success(let valid):
            ...
            let dest = subdir.appendingPathComponent(hash)
    ...
    private class PreviewCoordinator: NSObject, QLPreviewPanelDataSource {
        let url: URL
    ...
    private static var previewCoordinator: PreviewCoordinator?

    func quickLook(attachmentId: Int) {
        ...
        if sqlite3_step(stmt) == SQLITE_ROW {
            ...
            if let cext = sqlite3_column_text(stmt, 1) {
                ext = String(cString: cext)
            }
        }
        sqlite3_finalize(stmt)
        guard let hash = sha else { return }
        let base = attachmentsDir.appendingPathComponent(String(hash.prefix(2))).appendingPathComponent(hash)
        var displayURL = base
        if let ext = ext {
            let tmpDir = fm.temporaryDirectory.appendingPathComponent("DragonShieldPreview", isDirectory: true)
            try? fm.createDirectory(at: tmpDir, withIntermediateDirectories: true)
            let tmpFile = tmpDir.appendingPathComponent(hash).appendingPathExtension(ext)
            if fm.fileExists(atPath: tmpFile.path) { try? fm.removeItem(at: tmpFile) }
            try? fm.linkItem(at: base, to: tmpFile)
            displayURL = tmpFile
        }
        let coord = PreviewCoordinator(url: displayURL)
        AttachmentService.previewCoordinator = coord
        if let panel = QLPreviewPanel.shared() {
            panel.dataSource = coord
    ...
    func revealInFinder(attachmentId: Int) {
        guard let db = dbManager.db else { return }
        let sql = "SELECT sha256 FROM Attachment WHERE id = ?"
        var stmt: OpaquePointer?
        guard sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK else { return }
        sqlite3_bind_int(stmt, 1, Int32(attachmentId))
        var sha: String?
        ...
        sqlite3_finalize(stmt)
        guard let hash = sha else { return }
        let file = attachmentsDir.appendingPathComponent(String(hash.prefix(2))).appendingPathComponent(hash)
        NSWorkspace.shared.activateFileViewerSelecting([file])
    }

    func cleanupOrphans() -> Int {
        guard let db = dbManager.db else { return 0 }
        let sql = "SELECT id, sha256 FROM Attachment WHERE id NOT IN (SELECT attachment_id FROM ThemeUpdateAttachment)"
        var stmt: OpaquePointer?
        ...
        while sqlite3_step(stmt) == SQLITE_ROW {
            ...
            if let cStr = sqlite3_column_text(stmt, 1) {
                let sha = String(cString: cStr)
                let file = attachmentsDir.appendingPathComponent(String(sha.prefix(2))).appendingPathComponent(sha)
