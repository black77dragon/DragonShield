...
final class AttachmentService {
    ...
    func quickLook(attachmentId: Int) {
        guard let file = fileURL(for: attachmentId) else { return }
        AttachmentService.previewCoordinator.urls = [file]
    ...
    private func fileURL(for attachmentId: Int) -> URL? {
        guard let db = dbManager.db else { return nil }
        ...
        sqlite3_finalize(stmt)
        guard let hash = sha else { return nil }
        return attachmentsDir.appendingPathComponent(String(hash.prefix(2))).appendingPathComponent(hash)
    }
    ...
    @MainActor private static let previewCoordinator = PreviewCoordinator()
}

@MainActor
private final class PreviewCoordinator: NSObject, QLPreviewPanelDataSource {
    var urls: [URL] = []
