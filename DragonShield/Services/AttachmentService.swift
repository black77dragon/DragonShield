...
final class AttachmentService {
    ...
    func deleteAttachment(attachmentId: Int) -> Bool {
        guard let db = dbManager.db else { return false }
        var stmt: OpaquePointer?
        // Ensure no remaining links
        let countSQL = "SELECT COUNT(*) FROM ThemeUpdateAttachment WHERE attachment_id = ?"
        ...
        let selectSQL = "SELECT sha256, ext FROM Attachment WHERE id = ?"
        guard sqlite3_prepare_v2(db, selectSQL, -1, &stmt, nil) == SQLITE_OK else { return false }
        sqlite3_bind_int(stmt, 1, Int32(attachmentId))
        var sha: String?
        var ext: String?
        if sqlite3_step(stmt) == SQLITE_ROW {
        ...
        sqlite3_finalize(stmt)
        guard let hash = sha else { return false }
        let dir = attachmentsDir.appendingPathComponent(String(hash.prefix(2)), isDirectory: true)
        let file = dir.appendingPathComponent(hash + (ext.map { ".\($0)" } ?? ""))
