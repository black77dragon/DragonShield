    ...
    private var positionsTable: some View {
        ScrollView {
            Grid(horizontalSpacing: 16, verticalSpacing: 16) {
                ForEach(Array(viewModel.positions.enumerated()), id: \.element.id) { entry in
                    positionRow(index: entry.offset, position: entry.element)
                }
            }
...
private extension AccountDetailWindowView {
    @ViewBuilder
    func positionRow(index: Int, position: DatabaseManager.EditablePositionData) -> some View {
        if index < viewModel.positions.count {
            GridRow {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    Text("Quantity")
                        .font(.caption2)
                        .foregroundColor(.secondary)
                    TextField("", value: quantityBinding(for: index), formatter: Self.numberFormatter)
                        .frame(width: 80)
                }
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Button("Edit Price") { editingInstrument = InstrumentSheetTarget(id: position.instrumentId) }
                        .buttonStyle(.link)
                        .font(.caption)
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Price As Of")
                        .font(.caption2)
                        .foregroundColor(.secondary)
                    priceAsOfStyledText(for: position.instrumentUpdatedAt)
    ...
    func quantityBinding(for index: Int) -> Binding<Double> {
        Binding<Double>(
            get: {
                guard index < viewModel.positions.count else { return 0 }
                return viewModel.positions[index].quantity
            },
            set: { newValue in
                guard index < viewModel.positions.count else { return }
                var item = viewModel.positions[index]
                item.quantity = newValue
                viewModel.positions[index] = item
            }
        )
    }
        ...
        Binding<String>(
            ...
            set: { newValue in
                ...
                if trimmed.isEmpty {
                    ...
                    item.instrumentUpdatedAt = nil
                    viewModel.positions[index] = item
                } else if let value = Double(trimmed) {
                    guard index < viewModel.positions.count else { return }
                    var item = viewModel.positions[index]
                    item.currentPrice = value
                    item.instrumentUpdatedAt = Date()
                    viewModel.positions[index] = item
                }
            }
        )
