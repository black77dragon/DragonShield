...
extension DatabaseManager {
    ...
    func instrumentNoteCounts(instrumentId: Int, instrumentCode: String, instrumentName: String) -> (updates: Int, mentions: Int) {
        var updates = 0
        ...
        if sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
            ...
            LoggingService.shared.log("Failed to prepare instrumentNoteCounts updates: \(String(cString: sqlite3_errmsg(db)))", type: .error, logger: .database)
        }
        sqlite3_finalize(stmt)
