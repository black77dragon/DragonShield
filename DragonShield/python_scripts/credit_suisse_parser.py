...
def parse_date_from_excel_cell(
    cell_content: Any, input_format: Optional[str] = None
) -> Optional[str]:
    ...
    if isinstance(val, str):
        ...
        if input_format:
            try:
                ...
                pass
        match_dmY_short_long = re.match(
            r"(\d{1,2})\.(\d{1,2})\.(\d{2}|\d{4})", cell_str
        )
        if match_dmY_short_long:
            ...
            try:
                ...
                pass
        if (
            len(cell_str) >= 10 and cell_str[4] == "-" and cell_str[7] == "-"
...
def _find_instrument_by_valor(conn: sqlite3.Connection, valor: str) -> Optional[int]:
    search = _sanitize_key(valor)
    cur = conn.execute(
        "SELECT instrument_id, valor_nr FROM Instruments WHERE valor_nr IS NOT NULL"
    )
    for ins_id, db_val in cur.fetchall():
        if _sanitize_key(db_val) == search:
            return ins_id
    return None


def _find_instrument_by_isin(conn: sqlite3.Connection, isin: str) -> Optional[int]:
    search = _sanitize_key(isin)
    cur = conn.execute(
        "SELECT instrument_id, isin FROM Instruments WHERE isin IS NOT NULL"
    )
    for ins_id, db_isin in cur.fetchall():
        if _sanitize_key(db_isin) == search:
            return ins_id
    return None


def lookup_instrument(
    conn: sqlite3.Connection, name: str, valor: str, isin: str, logs: List[str]
) -> Optional[int]:
    if valor:
        ins_id = _find_instrument_by_valor(conn, valor)
        if ins_id is not None:
            logs.append(f"Matched instrument {name} (ID: {ins_id}) via valor")
            return ins_id
    if isin:
        ins_id = _find_instrument_by_isin(conn, isin)
        if ins_id is not None:
            logs.append(f"Matched instrument {name} (ID: {ins_id}) via ISIN")
            return ins_id
...
def parse_number_from_cell_value(cell_content: Any) -> Optional[float]:
    ...
    if isinstance(val, str):
        ...
        try:
            ...
            ValueError
        ):  # print(f"Warning: Could not parse number from string '{value_str}'") # Reduce noise
            return None
...
def process_file(filepath: str, sheet_name_or_index: Optional[Any] = None) -> int:
    ...
    parsed_data = {
        ...
        "parsed_statement_date": parse_statement_date_from_filename(
            filepath.split("/")[-1]
        ),  # Pass only filename
        "summary": {
    ...
    try:
        ...
        logs_list: List[str] = parsed_data["logs"]
        rows: List[List[Any]]
        if filepath.lower().endswith(".csv"):
            ...
            def cell(row: int, col: int):
                return (
                    rows[row - 1][col - 1]
                    if 0 <= row - 1 < len(rows) and 0 <= col - 1 < len(rows[row - 1])
                    else None
                )
            ...
            sheet = (
                workbook[sheet_name_or_index]
                if sheet_name_or_index is not None
                and isinstance(sheet_name_or_index, str)
                else (
                    workbook.worksheets[sheet_name_or_index]
                    if sheet_name_or_index is not None
                    and isinstance(sheet_name_or_index, int)
        ...
        for col_idx in range(1, min(max_column + 1, 6)):
            ...
            if parsed_nr:
                ...
                parsed_data["main_custody_account_nr"] = (
                    main_custody_account_nr_internal
                )
                break
        if not main_custody_account_nr_internal:
            print(
                f"Warning: Could not parse Account Nr from Line {LINE_6_PORTFOLIO_NR_LINE_NUMBER}.\n"
            )

        if sheet is None:
            header_cells = [
                cell(HEADER_ROW_NUMBER, i) for i in range(1, max_column + 1)
            ]
        ...
        idx_whrg_nominal = (
            whrg_indices[0] if len(whrg_indices) > 0 else IDX_WHRG_NOMINAL_FALLBACK
        )
        idx_whrg_kurs = (
            whrg_indices[1] if len(whrg_indices) > 1 else IDX_WHRG_KURS_FALLBACK
        )
        if idx_whrg_nominal == -1:
            print(
                "Warning: Could not find first 'Whrg.' column header for nominal currency."
            )
        if idx_whrg_kurs == -1:
            print(
                "Warning: Could not find second 'Whrg.' column header for price currency."
            )
        ...
        if sheet is None:
            ...
            row_iter = (
                (idx, [_get_actual_cell_value(c) for c in row])
        ...
        for row_idx_iter, row_cells_tuple in row_iter:
            ...
            def get_str_val_from_tuple(col_name, default_val=""):
                ...
                val = (
                    row_cells_tuple[col_idx]
                    if col_idx is not None and col_idx < len(row_cells_tuple)
                    else None
                )
                return str(val).strip() if val is not None else default_val

            def get_raw_val_from_tuple(
                col_name,
            ):  # Already returns primitive from values_only=True usually, but now from cell.value
                col_idx = header_map.get(col_name)
                if col_idx is not None and col_idx < len(row_cells_tuple):
            ...
            if (
                not anlagekategorie_str
                and not beschreibung_str
                and all(
                    c is None or str(c).strip() == ""
                    for c in row_cells_tuple[: min(5, len(row_cells_tuple))]
                )
            ):
                parsed_data["summary"]["skipped_footer_empty_rows"] += 1
                continue
            if (
                len(anlagekategorie_str) > 100
                or "real-time daten" in anlagekategorie_str.lower()
            ):
                parsed_data["summary"]["skipped_footer_empty_rows"] += 1
                continue
            ...
            record_data["main_custody_account_nr_from_file"] = (
                main_custody_account_nr_internal
            )
            asset_unterkategorie_str = get_str_val_from_tuple(COL_ASSET_UNTERKATEGORIE)
            mapped_group = get_mapped_instrument_group(
                anlagekategorie_str,
                asset_unterkategorie_str,
                unmapped_category_pairs_internal,
            )
            ...
            if asset_unterkategorie_str == "Konten":
                ...
                record_data["cash_account_number_from_file"] = get_str_val_from_tuple(
                    COL_VALOR
                )
                record_data["currency"] = (
                    str(row_cells_tuple[idx_whrg_nominal]).strip()
                    if idx_whrg_nominal != -1
                    and idx_whrg_nominal < len(row_cells_tuple)
                    and row_cells_tuple[idx_whrg_nominal] is not None
                    else None
                )
                record_data["balance"] = parse_number_from_cell_value(
                    get_raw_val_from_tuple(COL_ANZAHL_NOMINAL)
                )
                record_data["value_in_chf"] = parse_number_from_cell_value(
                    get_raw_val_from_tuple(COL_WERT_CHF)
                )
                record_data["fx_rate_to_chf"] = parse_number_from_cell_value(
                    get_raw_val_from_tuple(COL_DEVISENKURS)
                )
                ...
                record_data["quantity_nominal"] = parse_number_from_cell_value(
                    get_raw_val_from_tuple(COL_ANZAHL_NOMINAL)
                )
                cost_price_val = get_raw_val_from_tuple(COL_KOSTEN_KURS)
                record_data["cost_price"] = parse_number_from_cell_value(cost_price_val)
                if record_data["cost_price"] is not None:
                    parsed_data["summary"][
                        "instruments_with_cost_price"
                    ] += 1  # Corrected counter
                record_data["cost_price_currency"] = get_str_val_from_tuple(
                    COL_KOSTEN_KURS_WHRG
                )
                current_price_val = get_raw_val_from_tuple(COL_AKTUELLER_KURS)
                record_data["current_price"] = parse_number_from_cell_value(
                    current_price_val
                )
                record_data["current_price_currency"] = (
                    str(row_cells_tuple[idx_whrg_kurs]).strip()
                    if idx_whrg_kurs != -1
                    and idx_whrg_kurs < len(row_cells_tuple)
                    and row_cells_tuple[idx_whrg_kurs] is not None
                    else None
                )
                record_data["value_in_chf"] = parse_number_from_cell_value(
                    get_raw_val_from_tuple(COL_WERT_CHF)
                ...
                record_data["maturity_date"] = parse_date_from_excel_cell(
                    maturity_date_val, input_format="%d.%m.%y"
                )
                ...
                ins_id = lookup_instrument(
                    conn,
                    beschreibung_str,
                    record_data["valor_nr"],
                    record_data["isin"],
                    logs_list,
                )
                if ins_id is not None:
                    record_data["instrument_id"] = ins_id
                else:
                    unmatched_instruments += 1

            parsed_data["records"].append(record_data)

        parsed_data["summary"]["unmapped_categories"] = sorted(
            list(unmapped_category_pairs_internal)
        )
    ...
    # (Exception handling and JSON printing remain the same)
        ...
        parsed_data["summary"][
            "error"
        ] = "The 'openpyxl' library is required. Please install it (e.g., pip install openpyxl)."
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print(
            json.dumps(
                {
                    "error": "Please provide the XLSX file path as an argument.",
                    "usage": "python credit_suisse_parser.py <path_to_your_Credit-Suisse_file.xlsx>",
                }
            )
        )
        sys.exit(1)
