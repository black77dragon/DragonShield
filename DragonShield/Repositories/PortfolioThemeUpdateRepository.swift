...
final class PortfolioThemeUpdateRepository {
    ...
    func listThemeUpdates(themeId: Int, view: ThemeUpdateView = .active, typeId: Int? = nil, searchQuery: String? = nil, pinnedFirst: Bool = true) -> [PortfolioThemeUpdate] {
        ...
        let order = orderClause(for: view, pinnedFirst: pinnedFirst)
        ...
        if let db, sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
            if let stmt = stmt {
                bindFilters(stmt, values: binds)
                while sqlite3_step(stmt) == SQLITE_ROW {
                    items.append(readThemeUpdateRow(stmt))
                }
            }
        } else {
    ...
    func updateThemeUpdate(
        ...
        if let db, sqlite3_prepare_v2(db, "SELECT theme_id FROM PortfolioThemeUpdate WHERE id = ?", -1, &guardStmt, nil) == SQLITE_OK {
    ...
    func softDeleteThemeUpdate(id: Int, actor: String, source: String? = nil) -> Bool {
        var themeId = 0
        var stmt: OpaquePointer?
        if let db, sqlite3_prepare_v2(db, "SELECT theme_id FROM PortfolioThemeUpdate WHERE id = ?", -1, &stmt, nil) == SQLITE_OK {
            sqlite3_bind_int(stmt, 1, Int32(id))
    ...
    func listAllThemeUpdates(view: ThemeUpdateView = .active, typeId: Int? = nil, searchQuery: String? = nil, pinnedFirst: Bool = true) -> [PortfolioThemeUpdate] {
        var items: [PortfolioThemeUpdate] = []
        var clauses = ["u.soft_delete = \(view == .active ? 0 : 1)"]
        var binds: [Any] = []
        if let tid = typeId {
            clauses.append("u.type_id = ?")
            binds.append(tid)
    ...
    private func bindFilters(_ stmt: OpaquePointer, values: [Any]) {
        var index: Int32 = 1
        for b in values {
            if let i = b as? Int {
                sqlite3_bind_int(stmt, index, Int32(i))
    ...
    private func orderClause(for view: ThemeUpdateView, pinnedFirst: Bool) -> String {
        switch view {
        case .active:
            return pinnedFirst ? "u.pinned DESC, u.created_at DESC" : "u.created_at DESC"
        case .deleted:
            return "u.deleted_at DESC, u.created_at DESC"
        }
    }

    private func readThemeUpdateRow(_ stmt: OpaquePointer) -> PortfolioThemeUpdate {
