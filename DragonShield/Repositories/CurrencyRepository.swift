// DragonShield/Repositories/CurrencyRepository.swift
...
final class CurrencyRepository {
    ...
    func fetchActiveCurrencies() -> [(code: String, name: String, symbol: String)] {
        ...
        guard let db else { return currencies }
        let query = """
    ...
    func fetchCurrencyDetails(code: String) -> (code: String, name: String, symbol: String, isActive: Bool, apiSupported: Bool)? {
        ...
        var statement: OpaquePointer?
        if sqlite3_prepare_v2(db, query, -1, &statement, nil) == SQLITE_OK {
            // Using SQLITE_TRANSIENT tells SQLite to make its own copy of the string.
    ...
    func deleteCurrency(code: String) -> Bool {
        guard let db else { return false }
