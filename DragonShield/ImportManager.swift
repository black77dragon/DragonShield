               ...
               let sessionName = self.dbManager.nextImportSessionName(base: baseSessionName)
                ...
                if let sid = sessionId {
                    ...
                    self.dbManager.completeImportSession(id: sid,
                                                       ...
                                                       duplicateRows: 0,
                                                       notes: note)
                    let raw = self.dbManager.positionValuesForSession(sid)
                    self.dbManager.saveValueReport(raw, forSession: sid)
                    let items = self.dbManager.fetchValueReport(forSession: sid)
                    DispatchQueue.main.sync {
