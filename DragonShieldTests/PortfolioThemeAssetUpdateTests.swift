...
final class PortfolioThemeAssetUpdateTests: XCTestCase {
    ...
    func testListThemesForInstrumentWithCountsAndMentions() {
        ...
        _ = manager.createThemeUpdate(themeId: 1, title: "Astera", bodyMarkdown: "ALAB great", type: .General, pinned: false, author: "Ann", positionsAsOf: nil, totalValueChf: nil)
        _ = manager.createThemeUpdate(themeId: 2, title: "Note", bodyMarkdown: "Astera Labs Inc outlook", type: .General, pinned: false, author: "Ben", positionsAsOf: nil, totalValueChf: nil)
        let list = manager.listThemesForInstrumentWithCounts(instrumentId: 42, instrumentCode: "ALAB", instrumentName: "Astera Labs Inc")
        XCTAssertEqual(list.count, 2)
        let first = list.first { $0.themeId == 1 }
        XCTAssertEqual(first?.updatesCount, 2)
        XCTAssertEqual(first?.mentionsCount, 1)
        let second = list.first { $0.themeId == 2 }
        XCTAssertEqual(second?.updatesCount, 0)
        XCTAssertEqual(second?.mentionsCount, 1)
