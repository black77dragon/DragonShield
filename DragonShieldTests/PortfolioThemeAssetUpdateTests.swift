...
final class PortfolioThemeAssetUpdateTests: XCTestCase {
    ...
    func testCreateEditDeleteFlow() {
        ...
        var listOff = manager.listInstrumentUpdates(themeId: 1, instrumentId: 42, pinnedFirst: false)
        XCTAssertEqual(listOff.first?.id, second!.id)
        ...
        listOff = manager.listInstrumentUpdates(themeId: 1, instrumentId: 42, pinnedFirst: false)
        XCTAssertEqual(listOff.first?.id, second!.id)
